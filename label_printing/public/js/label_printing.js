...
// re-printed duplicates; the next call (from the same "Print" button, or
// "Print / Resume" on the job itself) simply continues from there.
    ...
    return new Promise(resolve => label_printing.browser_print.init(async () => {
        ...
        for (const row of rows) {
            try {
                ...
                frappe.msgprint(__("Printing stopped at serial {0}. Fix the printer and click the same Print button again (or Print / Resume on Label Print Job {1}) to continue with what's left.",[row.serial_no, job.name]));
                break;
            }
        }
...
label_printing.setup_print_buttons = async function(frm) {
    ...
    templates.filter(t => !t.source_child_table).forEach(template => {
        frm.add_custom_button(template.print_button_label || template.name, () => {
            label_printing.print_via_document_template(frm, template);
        }, __("Labels"));
    });
