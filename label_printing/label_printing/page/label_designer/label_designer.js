...
<div class="lp-designer">
  ...
  <div class="lp-work">
    ...
    const $r = $root, canvas = $r.find('.lp-canvas'), status = $r.find('.lp-status');
    ...
    function default_object(type, fieldname) {
        ...
        return {doctype:'Label Template Object', object_type:type, fieldname:fieldname || '', fixed_text:'', barcode_type:'Code 128', barcode_value_mode:'ERPNext Field', font:'0', font_size:28, alignment:'Left', x_mm:2 + (count % 4) * 4, y_mm:2 + Math.floor(count / 4) * 7, width_mm:type === 'Text' ? 35 : 25, height_mm:type === 'Text' ? 7 : 12, rotation:'0', data_matrix_scale:4, image_url:'', image_fit:'Contain', z_index:count + 1};
    }

    function add_object(type, fieldname) { push_history(); doc.objects.push(default_object(type, fieldname)); selected = doc.objects.length - 1; render(); }
    ...
    function render_properties() {
        ...
        b.find('[data-p]').on('change input',function(){ const p=$(this).attr('data-p'), old=o[p]; push_history(); o[p]=$(this).val(); if(['x_mm','y_mm','width_mm','height_mm','font_size','z_index'].includes(p)) o[p]=flt(o[p]); render(); });
        b.find('.lp-duplicate').on('click',()=>{push_history();const n=clone(o);n.x_mm=flt(n.x_mm)+2;n.y_mm=flt(n.y_mm)+2;n.z_index=(doc.objects.length+1);doc.objects.push(n);selected=doc.objects.length-1;render();});
    ...
    $(document).on('pointerup.label_printing_designer', function(){ if(drag||resize){drag=null;resize=null;render();} });
