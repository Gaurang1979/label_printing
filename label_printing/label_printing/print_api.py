    ...
    try:
        ...
        for yy in range(target_h):
            for bx in range(bpr):
                ...
                for bit in range(8):
                    xx = bx * 8 + bit
                    if xx < target_w and pixels[xx, yy] == 0:
                        value |= 1 << (7-bit)
                raw.append(value)
...
def render_label(template, parent, row=None, serial_no=None, printer=None):
    printer_doc = frappe.get_doc('Manage Printer', printer or template.printer)
    dpi = int(printer_doc.dpi or 203)
    ...
    for obj in sorted(template.objects, key=lambda x: x.z_index or 1):
        ...
        value = serial_no or value if obj.object_type in ('DataMatrix','QR Code') else value
        if obj.object_type == 'DataMatrix':
            zpl.append(datamatrix(obj.x_mm,obj.y_mm,value,obj.width_mm or 10,obj.rotation or '0',obj.data_matrix_scale or 5,dpi))
        elif obj.object_type == 'QR Code':
            zpl.append(qr(obj.x_mm,obj.y_mm,value,obj.width_mm or 10,obj.rotation or '0',obj.data_matrix_scale or 4,dpi))
        elif obj.object_type == 'Barcode':
            zpl.append(barcode(obj.x_mm,obj.y_mm,value,obj.barcode_type or 'Code 128',obj.height_mm or 12,obj.rotation or '0',True,dpi))
        elif obj.object_type == 'PDF417':
            zpl.append(pdf417(obj.x_mm,obj.y_mm,value,obj.width_mm or 10,obj.height_mm or 18,obj.rotation or '0',dpi))
        elif obj.object_type == 'Aztec':
            zpl.append(aztec(obj.x_mm,obj.y_mm,value,obj.rotation or '0',obj.data_matrix_scale or 3,dpi))
        elif obj.object_type == 'Image':
            zpl.append(_image_zpl(obj.image_url,obj.x_mm,obj.y_mm,obj.width_mm,obj.height_mm,dpi))
        elif obj.object_type == 'Text':
            zpl.append(text(obj.x_mm,obj.y_mm,value,obj.font_size or 28,obj.font or '0',obj.rotation or '0',obj.alignment or 'Left',dpi))
        elif obj.object_type == 'Line':
            zpl.append(line(obj.x_mm,obj.y_mm,obj.width_mm or 1,obj.height_mm or 0,obj.font_size or 2,dpi))
        elif obj.object_type == 'Rectangle':
            zpl.append(rectangle(obj.x_mm,obj.y_mm,obj.width_mm or 1,obj.height_mm or 1,obj.font_size or 2,dpi))
...
def create_print_job(source_doctype,source_name,template,printer,serials,child_table=None,child_row_idx=None,reprint=False,reprint_reason=None):
    ...
    if flt(template_doc.label_width_mm) > flt(frappe.db.get_value('Manage Printer',printer,'maximum_print_width_mm') or 0): frappe.throw(_('Label width exceeds printer maximum printable width.'))
