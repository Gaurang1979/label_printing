...
def render_label(template, parent, row=None, serial_no=None, printer=None):
    ...
    for obj in sorted(template.objects, key=lambda x: x.z_index or 1):
        ...
        if obj.object_type == "DataMatrix":
            zpl.append(datamatrix(obj.x_mm, obj.y_mm, serial_no or value, obj.width_mm or 10, obj.rotation or "0", obj.data_matrix_scale or 5, dpi))
        elif obj.object_type == "QR Code":
            zpl.append(qr(obj.x_mm, obj.y_mm, serial_no or value, obj.width_mm or 10, obj.rotation or "0", obj.data_matrix_scale or 4, dpi))
...
def preview_label(source_doctype, source_name, template, serial_no=None, child_table=None, child_row_idx=None, printer=None):
    parent = frappe.get_doc(source_doctype, source_name)
    ...
    if child_table and child_row_idx:
        row = next((r for r in (parent.get(child_table) or []) if int(r.idx) == int(child_row_idx)), None)
...
def create_print_job(source_doctype, source_name, template, printer, serials, child_table=None, child_row_idx=None, reprint=False, reprint_reason=None):
    ...
    for serial in serials:
        job.append("items", {"serial_no":serial, "status":"Pending", "child_row_idx": child_row_idx or 0})
...
def get_label_zpl(job, serial_no):
    ...
    if not row:
        frappe.throw(_("Serial {0} is not part of this print job.").format(serial_no))
    ...
    if template.source_child_table and row.child_row_idx:
        child_row = next((r for r in (source.get(template.source_child_table) or []) if int(r.idx) == int(row.child_row_idx)), None)
...
def mark_printed(job, serial_no):
    ...
    row.printed_on = frappe.utils.now_datetime()
    row.error_message = ""
...
def mark_failed(job, serial_no, error_message=None):
    ...
    row.status = "Failed"
    row.attempts = (row.attempts or 0) + 1
    row.error_message = error_message or _("Unknown printer error")
    job_doc.status = "Paused"
    job_doc.save(ignore_permissions=True)
    return True
...
def _refresh_job_status(job_doc):
    ...
    job_doc.printed_labels = printed
    job_doc.pending_labels = pending
